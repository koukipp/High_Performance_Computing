...
__global__ void convolutionRowGPU(double *d_Dst, double *d_Src, 
	int filterR,int row_tile_size) {

	int k;
	double sum = 0.0;
	int row = blockIdx.y*blockDim.y + threadIdx.y;
	int col = blockIdx.x*row_tile_size + threadIdx.x;
	int padding_skip = filterR * (2 * filterR + row_tile_size * gridDim.x) + filterR;
...
int main(int argc, char **argv) {
	...
	if(imageH < col_tile_size){
		col_tile_size = imageH;
		grid.x = 1;
		grid.y = 1;
	}
	else{
		grid.x = imageW/col_tile_size;
		grid.y = imageH/col_tile_size;
	}

	if(imageW < row_tile_size){
		row_tile_size = imageW;
	}
	printf("Col_tile_size:%d\n",col_tile_size);
	
	dim3 block;
	block.x = imageW/grid.x;
	block.y = imageH/grid.y;
	dim3 blockGridRows(imageW/row_tile_size, imageH);
